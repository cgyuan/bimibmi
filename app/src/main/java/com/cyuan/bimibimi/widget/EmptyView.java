...
public class EmptyView extends LinearLayout implements PlaceHolderView {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        mEmptyImg = findViewById(R.id.iv_empty);
        mStatusText = findViewById(R.id.tv_empty);
    ...
    public void triggerLoading() {
        mState = Constants.ViewState.LOADING;
    ...
    public void triggerOk() {
        mState = Constants.ViewState.DONE;
        setVisibility(GONE);
    ...
    public void setEmptyMessageId(int emptyMsgId) {
        if (emptyMsgId != 0) {
            mEmptyMessage = getResources().getString(emptyMsgId);
        }
    }
}